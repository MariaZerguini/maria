...
import { Ionicons } from '@expo/vector-icons'; // استخدمنا أيقونة السهم هنا
...
const AddExperienceScreen = () => {
  ...
  const handleSubmit = () => {
    ...
    route.params?.onAdd?.(newExperience);
    navigation.goBack();
  };
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
            {/* هنا استخدمنا أيقونة السهم المناسبة */}
            <Ionicons name="arrow-back" size={24} color="black" />
          </TouchableOpacity>

          <Text style={styles.title}>Add New Experience</Text> 
      ...
      <View style={styles.bottomButton}>
        <TouchableOpacity style={styles.button} onPress={handleSubmit}>
          <Text style={styles.buttonText}>Add to my experience</Text>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
